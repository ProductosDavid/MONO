...
class ActividadsController < ApplicationController
  ...
  def update
    ...
    id_nodo = @actividad.read_attribute(:my_js_tree_id)

    @my_js_tree = MyJsTree.find(id_nodo)
    @my_js_tree.update_attribute("title",params[:actividad][:nombre])

    id_proceso = @actividad.read_attribute(:proceso_id)

    if id_proceso != ""
      @tareas = Tarea.find_all_by_actividad_id(params[:id])
      @tareas.each do |tarea|
        tarea.update_attribute("nombre", params[:actividad][:nombre])
      end
    end
  ...
  def updateType
    ...
    if @actividad.update_attribute("modoejecucion", params[:nuevo_tipo])
