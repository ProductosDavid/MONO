...
class PlantillasController < ApplicationController
  ...
  def create
    if  params[:idTart] != '' then
      @plantilla = Plantilla.find(params[:idTart])
  ...
  def get_plantilla
    _id = params[:id]
    _plantilla = Plantilla.find(_id);

    @result = Hash.new

    @result[:nombre] = _plantilla.nombre;
    @result[:tipo_plantilla_id] = _plantilla.tipo_plantilla_id;
    @result[:descripcion] = _plantilla.descripcion;
    @result[:imagen] = _plantilla.imagen;
...
def existe
  ...
  def GenerateXPDL
    ...
    _xml = "<html><body>
            <a href=\"#{_url}/templates/proceso_out.xpdl\" />XPDL Model en formato XML </a> <br>
