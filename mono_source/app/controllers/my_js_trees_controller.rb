...
class MyJsTreesController < ApplicationController
  ...
  def jstreedata
    ...
    if(@id_plantilla)
      ...
      ordenar_arbol "plantilla" , params[:plantilla_id]
    else
      @all_ous =  MyJsTree.find_all_by_proceso_id(params[:proceso_id], :order =>"parent_id,position")#@plantilla.id)#params[:id_plantilla])# .find_all_by_plantilla_id(params[:id_plantilla])
      ordenar_arbol "proceso" , params[:proceso_id]
    end
